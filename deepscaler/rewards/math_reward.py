...
from deepscaler.globals import THOUGHT_DELIMITER_START, THOUGHT_DELIMITER_END, OAI_RM_MODEL, NO_THINK_PROMPT, NO_THINK_PROMPT_RESPONSE
...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        if NO_THINK_PROMPT in model_response or model_response.startswith(NO_THINK_PROMPT_RESPONSE):
            think = False
        else:
            think = True
        ...
        if THOUGHT_DELIMITER_END in model_response:  # for eval_model
            model_solution = model_response.split(THOUGHT_DELIMITER_END)[1]
        else:
            # model_solution = model_response
            return RewardOutput(reward=self.config.format_error_reward, is_correct=False, think=think)
        ...
        if ground_truths is None:
            return RewardOutput(reward=self.config.unk_error_reward, is_correct=False, think=think)
        ...
        for ground_truth in processed_ground_truths:
            ...
            if is_correct and think:
                return RewardOutput(reward=self.config.correct_reward, is_correct=True, think=think)
            elif is_correct and not think:
                return RewardOutput(reward=self.config.nothink_correct_reward, is_correct=True, think=think)
        ...
        return RewardOutput(reward=self.config.incorrect_reward, is_correct=False, think=think) if think else RewardOutput(reward=self.config.nothink_incorrect_reward, is_correct=False, think=think)
...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], val=False, enable_llm = False):
    ...
    if reward_response.reward < 0:
        ...
        if best_count >= 10:
            return -2.0, reward_response.is_correct, reward_response.think
        largest_word = find_largest_word(solution_str)
        if len(largest_word) >= 120:
            return -2.0, reward_response.is_correct, reward_response.think
    return reward_response.reward, reward_response.is_correct, reward_response.think  #reward = 0 / 1

        # reward = reward_response.reward + 1 if reward_response.is_correct else reward_response.reward - 1  # 2 / -1
        # return reward, reward_response.is_correct, reward_response.think
